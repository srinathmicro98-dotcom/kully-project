...
import { SCRAPE_TOOL, handleScrapeTool, RECALL_ACROSS_PROJECTS_TOOL, handleRecallAcrossProjectsTool } from './sharedTools.js';
...
snippet alone isn't enough to answer well — use it rather than guessing from the snippet. If the question \
is actually about the user's OWN past notes/decisions (not live external info), use recall_across_projects \
instead of treating the web results below as the answer. \
...
const TOOLS = [SCRAPE_TOOL, RECALL_ACROSS_PROJECTS_TOOL];
...
export async function handle(ctx) {
  ...
  const reply = await runToolLoop({ messages, tools: TOOLS, dispatch: (call) => dispatch(call, ctx), maxIterations: 3, temperature: 0.3, userId: ctx.userId });
  return { reply };
}
